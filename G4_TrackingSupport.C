...
double TrackingService(PHG4Reco *g4Reco, double radius)
{
  ...
  cones.push_back(new ServiceProperties("ETrackingCone_Disk4TouRwell", CuThickness, AlThickness, WaterThickness, PlasticThickness, CarbonThickness, 0, -147.47, -90, 68, 40.));

  // The cylindrical plateau structure for Electron uRwell side.
  cylinders.push_back(new ServiceProperties("ETrackingCyl_uRWellPlateau", CuThickness, AlThickness, WaterThickness, PlasticThickness, CarbonThickness, 0, -90, -40., 40., 0));

  // Cone service from electron side uRwell1 to vertex support.
  cones.push_back(new ServiceProperties("ETrackingCone_uRwellToVertex", CuThickness, AlThickness, WaterThickness, PlasticThickness, CarbonThickness, 0, -40., -13.5, 40., 13.5));

  // Sagitta Cylindrical Support Structure
  cylinders.push_back(new ServiceProperties("BTrackingCyl_Vertex2", 0, 0, 0, 0, X_X0_percent, 0, -13.6, 13.6, 13.6, 0));

  // Cone from second silicon layer to third silicon layer in E region
  cones.push_back(new ServiceProperties("E_si2_to_si3", 0, 0, 0, 0, X_X0_percent, 0, -13.6,-13.5, 13.6, 5.7));

  // Vertex Cylindrical Support Structure
  cylinders.push_back(new ServiceProperties("BTrackingCyl_Vertex", 0, 0, 0, 0, X_X0_percent, 0, -13.5,13.5, 5.7, 0));

  // Cone from second silicon layer to third silicon layer in H region
  cones.push_back(new ServiceProperties("H_si2_to_si3", 0, 0, 0, 0, X_X0_percent, 0, 13.5, 13.6, 5.7, 13.6));

  // Cone service in H-region from vertex to inner uRwell
  cones.push_back(new ServiceProperties("HTrackingCone_VertexTouRwell", CuThickness, AlThickness, WaterThickness, PlasticThickness, CarbonThickness, 0, 13.5, 40., 13.5, 40.));

  // Cylinder service to rest the uRwell in H region Plateau
  cylinders.push_back(new ServiceProperties("HTrackingRWellPlateau", CuThickness, AlThickness, WaterThickness, PlasticThickness, CarbonThickness, 0, 40.0, 103.75, 40.0, 0));

  // Cone service from uRwell to Disk 5 in h-region
  cones.push_back(new ServiceProperties("HTrackingCone_uRwellToDisk5", CuThickness, AlThickness, WaterThickness, PlasticThickness, CarbonThickness, 0, 103.75, 173, 40.0, 69.2));
